...
test('beta display version is consistent across release-facing files', () => {
  ...
  const releaseFiles = ['README.md', 'STORE-LISTING.txt', 'popup.html', 'styles.css'];

  assert.equal(manifest.version_name, '6.3.0-beta.5');
  for (const file of releaseFiles) {
    assert.match(read(file), /6\.3\.0-beta\.5/, `${file} should identify beta.5`);
  }
  for (const file of ['STORE-LISTING.txt', 'popup.html', 'styles.css']) {
    assert.doesNotMatch(read(file), /6\.3\.0-beta\.4/, `${file} should not display the previous beta`);
  }
});
...
test('store listing explains every requested permission', () => {
  ...
  assert.match(listing, /dist\/swipe-for-letterboxd-v6\.3\.0-beta\.5\.zip/);
